...
Status: completed on 2026-06-28. Standalone image OCR remains a later milestone.

Implementation checkpoint:

- Chose a bundled local execution path: `TesseractOCR` 5.5.2 supplies the native OCR worker and `PDFtoImage` 5.2.1 supplies bounded PDFium/Skia rasterization. OCR remains disabled by default and requires no separately installed Tesseract or Poppler executable.
- Added `[ocr]` config with `enabled`, `auto_download_language_packs`, `languages`, and `fallback_script`. `status` and `email_get_setup_status` report the configured mode, validation errors, and cached models.
- Solved the image-only language bootstrap problem with two modes. Configured Tesseract language codes such as `eng`, `srp`, and `srp_latn` download only those smaller models and are preferred for a known corpus. An empty language list downloads `osd.traineddata`, detects the script, and then downloads one broad script model such as Latin or Cyrillic. Exact language cannot reliably be detected before OCR because the page has no text to classify.
- Language/script models download lazily only from commit `87416418657359cb625c412a48b6e1d6d41c29bd` of Tesseract's official `tessdata_fast` GitHub repository. Downloads use fixed app-owned URLs, a 100 MiB per-model cap, atomic temp writes, and the app-managed `ocr/tessdata` cache. Disabling automatic downloads turns a missing model into an explicit terminal issue.
- PDF extraction now identifies empty, tiny, or suspicious pages while retaining useful PdfPig text from mixed PDFs. OCR text is persisted in the existing `ocr_text`/`combined_text` path, so attachment FTS, snippets, readiness, and `email_get_attachment_text` require no separate OCR-facing API.
- Rendering is bounded to 200 DPI, 20 OCR-candidate pages, 6,000 pixels per dimension, 20 million pixels per page, and 25 MiB per encoded rendered page. OCR-enabled terminal work has the existing single-runaway-worker gate and a two-minute caller timeout; raster pages stay in memory so there are no page temp files to leak.
- OCR-disabled candidates, missing models, unavailable native workers/renderers, corrupt/encrypted PDFs, timeouts, and safety-limit rejections use stable domain errors in the item 9 attempt/failure lifecycle. Enabling OCR queues prior successful PDFs from older processor versions and prior `ocr_disabled` failures as `extractor_upgrade` work while preserving stable attachment IDs.
- Follow-up on 2026-06-29: added MCP OCR config tooling. `email_list_ocr_languages` reports supported language codes and script models from the pinned official tessdata revision plus current OCR/cache status. `email_set_ocr_config` updates `enabled`, `languages`, `fallback_script`, and the offline/preseed policy flag without a server restart; settings apply to the next sync, retry, or extraction run. The setter validates against the pinned catalog, treats `languages = []` as auto-script mode, normalizes valid codes, rejects unsupported codes without saving, and warns when automatic downloads are disabled but expected models are not cached.
- `dotnet test lcemcp.slnx --no-restore /p:UseSharedCompilation=false` passed with 90 tests after the MCP OCR config tools were added. `dotnet format lcemcp.slnx --verify-no-changes --no-restore` and `git diff --check` passed.
- Live local Yahoo OCR backfill on 2026-06-29: enabled OCR with `eng`, `srp`, and `srp_latn`, lazily downloaded/cache-populated those three language models, and ran repeated `sync-bodies --account yahoo --max-per-folder 0 --batch-size 10` passes. The first pass queued 173 OCR-capable PDF re-extractions; successive batches processed 50, 50, 50, 27, and confirmation batches of 4 with 0 OCR failures. Final `status` reported OCR enabled, cached models `eng, srp, srp_latn`, 174 attachment text rows, 0 pending attachment work, attachment index complete, and unchanged 39 open `unsupported_attachment_type` failures.
- Follow-up bug found and fixed during the live OCR backfill: 4 OCR'd PDFs were requeued on every `sync-bodies` run even though they processed successfully and status returned to clean afterward. Confirmed from metadata only that the rows had `extractor = PdfPig+Tesseract(...)`, `ocr_text_available = 1`, and the native OCR toolchain string in `extractor_version`, while `QueuePdfOcrUpgradeCandidates` correctly expected the attachment processor generation (`"2"`). Corrected the model so `extractor_version` is only the lcemcp attachment processor generation; OCR/PDFium/tessdata provenance is kept in the readable `extractor` string. A one-time live cleanup run reprocessed the 4 rows and stamped them with processor version 2; the next live run selected `0/0` work.
- `dotnet test lcemcp.slnx --no-restore /p:UseSharedCompilation=false` passed with 90 tests after the repeat-OCR queue fix. `dotnet format lcemcp.slnx --verify-no-changes --no-restore` and `git diff --check` passed.
- Added image-only, mixed embedded/scanned, OCR-disabled, timeout, page-limit, model download/cache, config round-trip, upgrade-trigger, and stale-FTS cleanup coverage. The full suite passed with 87 tests; format verification and diff checks passed.
- Manual real-worker smoke tests recognized an image-only PDF using both a lazily downloaded configured `eng` model and automatic OSD-to-Latin script selection. An isolated `status` smoke test created schema version `9 / target 9` and reported OCR disabled with no cached models on a fresh config.
